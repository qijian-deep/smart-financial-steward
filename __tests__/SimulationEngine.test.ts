...
describe('SimulationEngine.calculate', () => {
  ...
  describe('单只基金 + 月收入 + 月支出', () => {
    ...
    it('应该正确计算单只基金的收益', () => {
      ...
      // 验证总资产被计算
      expect(firstMonth.totalAssets).toBe(20000)

      const secondMonth = result!.monthlyData[1]
      expect(secondMonth.month).toBe('2015-02')
      // 第二个月累计投入 = 第一个月累计投入 + 月收入 - 月支出 = 15000 + 10000 - 5000 = 20000
      expect(secondMonth.cumulativeInvestment).toBe(20000)
      // 验证基金收益被计算 (10000 + 5000) * 0.75
      expect(secondMonth.totalAssets).toBe(21250)

      const thirdMonth = result!.monthlyData[2]
      expect(thirdMonth.month).toBe('2015-03')
      // 第三个月累计投入 = 第一个月累计投入 + 月收入 - 月支出 = 15000 + 10000 - 5000 = 20000
      expect(thirdMonth.cumulativeInvestment).toBe(20000 + 5000)
      // 验证基金收益被计算
      expect(thirdMonth.totalAssets).toBe(58750)
